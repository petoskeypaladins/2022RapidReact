...
public class AutoMoveForwardPIDCommand extends CommandBase {
  /** Creates a new AutoPIDForwardCommand. */
  private double goalOne;
  public AutoMoveForwardPIDCommand() {
  ...
  public AutoMoveForwardPIDCommand(double a) {
    // Use addRequirements() here to declare subsystem dependencies.
    addRequirements(RobotContainer.m_DriveSubsystem);
    goalOne = a;
  }
  ...
  public void execute() {
    ...
    if (DriveSubsystem.pidOne.calculate(DriveSubsystem.leftEncoder.getPosition(), goalOne) >= 0) {
      DriveSubsystem.drive.arcadeDrive(0.24 +(DriveSubsystem.pidOne.calculate(DriveSubsystem.leftEncoder.getPosition(), goalOne)*0.6), 0);
    } else {
      DriveSubsystem.drive.arcadeDrive(-0.24 +(DriveSubsystem.pidOne.calculate(DriveSubsystem.leftEncoder.getPosition(), goalOne)*0.6), 0);
    }
 
 
    //DriveSubsystem.pidOne.calculate([current position], [set point]);
