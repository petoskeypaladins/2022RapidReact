...
public class AutoTurnRightPIDCommand extends CommandBase {
  /** Creates a new AutoPIDForwardCommand. */
  private double goalOne;
  private double speed;
  public AutoTurnRightPIDCommand() {
  ...
  public AutoTurnRightPIDCommand(double a, double s) {
    // Use addRequirements() here to declare subsystem dependencies.
    addRequirements(RobotContainer.m_DriveSubsystem);
    goalOne = a;
    speed = s;
  ...
  public void execute() {
    ...
    if (DriveSubsystem.pidOne.calculate(DriveSubsystem.leftEncoder.getPosition(), goalOne) >= 0) {
      DriveSubsystem.drive.arcadeDrive(0, 0.3 +(DriveSubsystem.pidOne.calculate(DriveSubsystem.leftEncoder.getPosition(), goalOne)*speed));
    } else {
      DriveSubsystem.drive.arcadeDrive(0, -0.3 +(DriveSubsystem.pidOne.calculate(DriveSubsystem.leftEncoder.getPosition(), goalOne)*speed));
    }
  }
